 ...
 * @author bizcho
 */
public class CancionConsulta extends JFrame {
    ...
    private String[] titulos = {"ID Cancion", "Posicion", "Titulo", "Genero", "Disco"};

    public CancionConsulta() {
        super("Cancion consulta");
        //auxiliares
        ...
        tBuscar.addKeyListener(new CancionConsulta.Buscador(this));
    ...
    private class Buscador implements KeyListener {
        ...
        CancionConsulta consulta;

        Buscador(CancionConsulta consulta) {
    ...
    private void buscar() {
        ...
        if (seleccion == JOptionPane.YES_OPTION) {
            if (campo != 0) {
                ResultSet rs = null;
                try {
                    rs = Conexciones.search(Conexciones.CANCION, getColumn(campo), dato);
                } catch (Exception e) {
    ...
    private TableModel mkModelo(ResultSet rs) {
        ...
        try {
            if (rs != null) {
                while (rs.next()) {
                    campos[0] = rs.getString("idcancion");
                    campos[1] = rs.getString("posicion");
                    campos[2] = rs.getString("titulo");
                    campos[3] = rs.getString("genero");
                    campos[4] = rs.getString("titulod");
    ...
    TableModel getModelo(String consulta) {
        ...
        ResultSet rs = null;
        try {
            rs = (Conexciones.searchData(Conexciones.CANCION, consulta));
            if (rs != null) {
                while (rs.next()) {
                    campos[0] = rs.getString("idcancion");
                    campos[1] = rs.getString("posicion");
    ...
    private void cambiar() {
        ...
        if (fila != -1) {
            ...
            CancionCambios ventana = new CancionCambios(this, datos);
        } else {
