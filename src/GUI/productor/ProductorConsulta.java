 ...
 * @author bizcho
 */
public class ProductorConsulta extends JFrame {
    ...
    private String[] titulos = {"ID disco", "Nombre",};

    public ProductorConsulta() {
        super("Productor consultas");
        //auxiliares
        ...
        tBuscar.addKeyListener(new ProductorConsulta.Buscador(this));
    ...
    private class Buscador implements KeyListener {
        ...
        ProductorConsulta consulta;

        Buscador(ProductorConsulta consulta) {
    ...
    private void buscar() {
        String[] titulos = {"Seleccione campo", this.titulos[0], this.titulos[1]};
        boolean aux = true;
        int seleccion;
        ...
        if (seleccion == JOptionPane.YES_OPTION) {
            ...
            if (campo != 0) {
                ResultSet rs = null;
                try {
                    rs = Conexciones.search(Conexciones.PRODUCTOR, getColumn(campo), dato);
                } catch (Exception e) {
    ...
    private TableModel mkModelo(ResultSet rs) {
        ...
        try {
            if (rs != null) {
                while (rs.next()) {
                    campos[0] = rs.getString("idproductor");
    ...
    TableModel getModelo(String consulta) {
        ...
        ResultSet rs = null;
        try {
            rs = (Conexciones.searchData(Conexciones.PRODUCTOR, consulta));
            if (rs != null) {
                while (rs.next()) {
                    campos[0] = rs.getString("idproductor");
                    campos[1] = rs.getString("nombre");
    ...
    private void cambiar() {
        ...
        if (fila != -1) {
            ...
            ProductorCambios ventana = new ProductorCambios(this, datos);
        } else {
